...
(async()=>{
  ...
  if(!MV.heroArt) throw new Error('MV.heroArt missing -> pickBossMove returns null and hero bosses never attack');
  ['hbInu','hbShima','hbNuko','hbGuard8','hbWatch','hbWanden'].forEach(k=>{
    if(!ETYPE[k].aggro) throw new Error(k+' lacks the aggro flag'); });
  ...
  if(pickBossMove(hb,120)!=='heroArt') throw new Error('hero boss has no usable move at close range');
  if(pickBossMove(hb,900)!=='heroArt') throw new Error('hero boss has no usable move at long range');
  ...
  if(starts<8) throw new Error('hero boss barely attacked ('+starts+' moves in 600 frames)');
  if(used.size<2) throw new Error('hero boss repeats a single move ('+[...used].join(',')+')');
  if(dmgTaken<=0) throw new Error('hero boss never damaged the player');
  ...
  if(hb2.state!=='air') throw new Error('hero boss should still tumble like a player');
  ...
  ['megaBeam','skyDive','bladeParry','bladeCounter','bulletHell','boltStrike','megaTornado'].forEach(m=>{
    if(!MV[m]) throw new Error('MV.'+m+' missing'); });
  ...
  if(!BOSSMOVES.mumei.includes(pickBossMove(enemies[0],120))) throw new Error('per-type moveset not used for mumei');
  ...
  if(!beam) throw new Error('megaBeam produced no beam');
  const beamDmg=hurtBy(()=>{ for(let i=0;i<12;i++){ ready(); updateHazards(); } });
  if(beamDmg<=0) throw new Error('megaBeam did not hit the player');
  console.log('極太ビーム OK (進路上を薙ぎ払い '+beamDmg+' ダメージ)');

  // 剣士：急降下突進
  const sd=runMove('boss3','skyDive');
  if(!sd.slammed) throw new Error('skyDive never landed');
  if(sd.z>1) throw new Error('skyDive did not return to the ground (z='+sd.z+')');
  ...
  if(!(gx.parryStance>gf)) throw new Error('bladeParry did not enter the stance');
  curHitIsMelee=true; damageEnemy(gx, 10, 5, false, 0); curHitIsMelee=false;
  if(gx.moveName!=='bladeCounter') throw new Error('parry did not trigger the counter (move='+gx.moveName+')');
  const cDmg=hurtBy(()=>{ for(let f=0;f<MV.bladeCounter.dur;f++){ ready(); gx.moveT=f; runBossMove(gx); } });
  if(cDmg<=0) throw new Error('bladeCounter dealt no damage');
  console.log('見切りカウンター OK (構え→近接を受けて返し刃、巨大な一閃で '+cDmg+' ダメージ)');

  // 飛び道具：弾幕
  runMove('cactus','bulletHell');
  if(projectiles.length<30) throw new Error('bulletHell fired only '+projectiles.length+' shots');
  console.log('弾幕 OK ('+projectiles.length+' 発の波状射撃)');

  // 魔法使い：落雷
  runMove('noroinu','boltStrike');
  const bolts=hazards.filter(h=>h.kind==='ebolt');
  if(bolts.length<4) throw new Error('boltStrike dropped only '+bolts.length+' bolts');
  const boltDmg=hurtBy(()=>{ for(let i=0;i<30;i++){ ready(); updateHazards(); } });
  if(boltDmg<=0) throw new Error('boltStrike never connected');
  console.log('落雷 OK ('+bolts.length+' 本を狙って落とし '+boltDmg+' ダメージ)');

  // 魔法使い：巨大竜巻
  runMove('noroinu','megaTornado');
  const tw=hazards.find(h=>h.kind==='etwister');
  if(!tw) throw new Error('megaTornado spawned no twister');
  const pxBefore=p.x;
  const twDmg=hurtBy(()=>{ for(let i=0;i<40;i++){ ready(); updateHazards(); } });
  if(twDmg<=0) throw new Error('megaTornado never connected');
  if(Math.abs(p.x-pxBefore)<10) throw new Error('megaTornado did not pull the player in');
