...
(function (g) {
  ...
  ASH.enemy = function (T, mats, type) {
    ...
    if (!isMark) {
      ...
      /* 前傾0.30(約17度)＋無首＋腰より低い拳の組み合わせが、
         「猫背・無首・長腕の獣」という参照作品の種族シルエットに寄る、と
         参照境界監査に指摘された。世界観は「両陣営とも人間」と宣言しているので、
         姿勢の側でそれを裏切ってはいけない。前傾を緩め、首を見せ、拳を腰より上へ。
         幅は読解性（突撃型＝横に広い）のために据え置く。獣性の署名は幅ではなく姿勢。*/
      var LEAN = 0.20, LC = Math.cos(LEAN), LS = Math.sin(LEAN);
      ...
      /* 頸部。無首は獣性の最大の署名なので、肩の谷から首を1段出す。 */
      tB.add(BOX, 0.15, 0.14, 0.15, 0, 0.42, 0.06, 0, 0, 0, DK);
      tB.add(BOX, 0.28, 0.34, 0.30, 0, 0.58, 0.10, 0, 0, 0, DK);
      ...
      /* 拳が腰より低いのも獣性の署名。前腕から先を持ち上げる。 */
      aB.add(BOX, 0.21, 0.20, 0.24, 0.01, -0.48, 0.30, 0, 0, 0, AR);
      aB.add(BOX, 0.15, 0.16, 0.30, 0.01, -0.44, 0.46, 0, 0, 0, DK);
      aB.add(CYL, 0.11, 0.14, 0.11, 0.01, -0.44, 0.66, Math.PI / 2, 0, 0, TR);
