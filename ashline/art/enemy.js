  ...
  ASH.enemy = function (T, mats, type) {
    ...
    if (!isMark) {
      /* -----------------------------------------------------------------
         ...
         ---------------------------------------------------------------- */
      /* 前傾 28.6°。胴を倒すほど全高が下がり、同時に頭と肩が腰より前に出る。
         「低い」と「重心が前」を1つのパラメータで同時に作れるので角度は強めに取る。 */
      torso.rotation.x = 0.50;
      /* 肩ピボットを外へ逃がして、胸の側面と上腕の間に切れ込みを作る。
         ここが繋がると輪郭が1枚の塊になり人型に見えなくなる。 */
      armR.position.set(0.50, 0.14, 0);
      armL.position.set(-0.50, 0.14, 0);
      ...
      tB.add(BOX, 0.62, 0.44, 0.44, 0, 0.16, 0.00, 0, 0, 0, DK);
      tB.add(BOX, 0.70, 0.40, 0.15, 0, 0.22, 0.25, -0.20, 0, 0, AR);
      tB.add(BOX, 0.66, 0.24, 0.13, 0, -0.02, 0.27, 0.26, 0, 0, AR);
      tB.add(BOX, 0.72, 0.05, 0.11, 0, 0.09, 0.30, 0, 0, 0, TR);
      tB.mirror(BOX, 0.09, 0.36, 0.38, 0.32, 0.14, 0.02, 0, 0, 0, AR);
      ...
      tB.add(BOX, 0.74, 0.18, 0.40, 0, 0.38, 0.00, 0, 0, 0, DK);
      tB.mirror(BOX, 0.30, 0.36, 0.44, 0.48, 0.32, 0.02, 0, 0, -0.42, AR);
      tB.mirror(BOX, 0.32, 0.05, 0.42, 0.49, 0.47, 0.02, 0, 0, -0.42, TR);
      tB.mirror(BOX, 0.20, 0.18, 0.22, 0.47, 0.30, 0.28, -0.20, 0, -0.42, AR);
      /* 頭。肩甲板の谷から 0.12m だけ出る細い塊。首は作らない。
         幅を肩の切れ込みより狭くしないと輪郭で頭が消え、人型に見えなくなる。 */
      tB.add(BOX, 0.23, 0.30, 0.32, 0, 0.58, 0.15, 0, 0, 0, DK);
      tB.add(BOX, 0.19, 0.09, 0.11, 0, 0.60, 0.31, 0, 0, 0, TR);
      tB.add(BOX, 0.19, 0.11, 0.17, 0, 0.47, 0.27, 0, 0, 0, AR);
      /* 左前腕＝衝角板（胴と一体成型。armL は肩ピボットとして空で残す） */
      tB.add(BOX, 0.22, 0.34, 0.26, -0.50, 0.00, 0.04, 0, 0, 0, DK);
      tB.add(BOX, 0.19, 0.46, 0.34, -0.55, -0.16, 0.16, -0.12, 0, 0, AR);
      tB.add(BOX, 0.06, 0.44, 0.32, -0.65, -0.16, 0.16, -0.12, 0, 0, TR);
      tB.add(BOX, 0.21, 0.18, 0.24, -0.50, -0.42, 0.28, 0, 0, 0, DK);

      /* --- 右腕。拳を腰より低く前に置き、短い筒だけを持たせる。
         長い銃身を持たせないことが marksman との一目での差になる。 --- */
      aB = new Bld(T, base, 0.98 + 0.14);
      aB.add(BOX, 0.22, 0.34, 0.26, 0.00, -0.14, 0.04, 0, 0, 0, DK);
      aB.add(BOX, 0.24, 0.16, 0.26, 0.01, -0.33, 0.08, 0, 0, 0, AR);
      aB.add(BOX, 0.20, 0.32, 0.22, 0.01, -0.49, 0.18, -0.40, 0, 0, DK);
      aB.add(BOX, 0.22, 0.20, 0.24, 0.01, -0.63, 0.30, 0, 0, 0, AR);
      ...
      gun.position.set(0.51, -0.45, 0.46);
