...
fail=0
for t in rpg feat watch feat2 ng2 sg wanden ng3 boss lvup evo foeatk fix myth grow steam anim anim2 bg crit train voice sengoku mack terr combo cos wpn; do
...
echo "---"
[ $fail -eq 0 ] && echo "ALL 29 SUITES PASSED" || echo "SOME SUITES FAILED"
exit $fail
