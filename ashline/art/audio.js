  ...
  ASH.audio = function () {
    ...
    function makePlazaIR(sr) {
      ...
      for (ch = 0; ch < 2; ch++) {
        ...
        /* --- 拡散音場（薄い） ---------------------------------------------
           90ms から立ち上げる。それより前は「数えられるタップ」の領域で、
           そこに拡散を入れると途端に室内になる。
           振幅 0.085 はタップのピーク（約0.5）の 1/6。上に抜けるので育たない。 */
        t0 = 0.090;
        i = Math.floor(t0 * sr);
        lp = 0;
        var hpS = 0;
    ...
    function build() {
      ...
      /* 致命打などで一瞬だけ世界を引かせるサイドチェイン用。 */
      duckG = gain(1.0);
      duckG.connect(master);

      dryBus = gain(1.0);
      dryBus.connect(duckG);
    ...
    function shot(kind) {
      ...
      /* [2b] ボディ：低域サイン。ブラインドは低域だけが回り込むので相対的に増やす */
      o = L();
      o.hz = 82 * r; o.hz2 = 54 * r;
      o.atk = 0.002; o.dec = 0.145;
      o.pk = blind ? 0.70 : 0.55;
      lay(v, 2, o);
    ...
    function impact(kind, x, y, z) {
      ...
      if (kind === 'enemy' || kind === 'head') {
        ...
        if (world) placeWorld(v, x, y, z); else place(v, 0, 0, 7.0);
        ...
        if (head) {
          ...
          o = L(); o.buf = NZP; o.ft = 'bandpass';
          o.fa = 360 * r; o.q = 9.0;
          o.atk = 0.003; o.dec = 0.34; o.pk = 0.30;
          lay(v, 3, o);
          ...
          o.del = 0.085; o.atk = 0.006; o.dec = 0.35; o.pk = 0.34;
          /* 声を1本余計に取らず、空きブランチが無ければ諦める設計にはせず、
             ここは別の声に載せる（4ブランチを使い切っているため）。 */
          var v2 = alloc(0.55);
          if (world) placeWorld(v2, x, y, z); else place(v2, 0, 0, 7.0);
          lay(v2, 0, o);
          send(v2, 0.20, 0.14, 0);
          ...
          o.fa = 1800 * r; o.q = 0.8;
          o.atk = 0.001; o.dec = 0.055; o.pk = 0.22;
          lay(v, 3, o);
        }
      ...
      o.q = 0.8; o.atk = 0.0004; o.dec = 0.0045; o.pk = 0.42;
      lay(v, 0, o);

      /* 欠けた破片。1.9k→1.2kHz へ落ちる短い帯域＝「かつん」。 */
      o = L(); o.buf = NZW; o.ft = 'bandpass';
      o.fa = 1900 * r; o.fb = 1200 * r; o.q = 1.6;
      o.atk = 0.0010; o.dec = 0.042; o.pk = 0.34;
      lay(v, 1, o);

      /* 砕けた粉が落ちる余韻。これがあると「石」になる。 */
      o = L(); o.buf = NZP; o.ft = 'bandpass';
      o.fa = 620 * r; o.q = 0.9;
      o.atk = 0.002; o.dec = 0.075; o.pk = 0.16;
      lay(v, 2, o);
    ...
    function vault() {
      ...
      o.atk = 0.020; o.dec = 0.120; o.pk = 0.14;
      lay(v, 0, o);
    ...
    function step(speed) {
      ...
      /* 踵。速いほど短く硬く。 */
      o = L(); o.buf = NZW; o.ft = 'highpass'; o.fa = 2500 * r; o.q = 0.8;
      o.atk = 0.0004; o.dec = 0.0035 + 0.0025 * (1 - sp); o.pk = 1.00 * lvl;
      lay(v, 0, o);

      /* 靴底が石を押す本体。430Hz は石畳を靴で叩いたときの主要な帯域。 */
      o = L(); o.buf = NZP; o.ft = 'bandpass';
      o.fa = 430 * r; o.fb = 300 * r; o.q = 1.3;
      o.atk = 0.001; o.dec = 0.045 + 0.020 * (1 - sp); o.pk = 0.85 * lvl;
      lay(v, 1, o);

      /* 砂と欠片の擦れ。走るほど増える。砲撃の翌日の広場なので必ず在る。 */
      o = L(); o.buf = NZW; o.ft = 'highpass'; o.fa = 3600 * r; o.q = 0.7;
      o.atk = 0.003; o.dec = 0.055; o.pk = (0.25 + 0.55 * sp) * lvl;
      lay(v, 2, o);
