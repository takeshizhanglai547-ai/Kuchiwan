...
(function (g) {
  ...
  ASH.player = function (T, mats) {
    ...
    box(bT, CAD, 0, tw(1.555), -0.01, 0.34, 0.10, 0.30, { tx: 0.82, ct: CAD });
    ...
    /* 背嚢一式：背面シルエットの主役 */
    box(bT, CC, 0, tw(1.36), -0.30, 0.38, 0.34, 0.20, { tx: 0.90, tz: 0.85, ct: CC2 });
    box(bT, CC2, 0, tw(1.545), -0.295, 0.35, 0.05, 0.185);
    tube(bT, CC2, 0, tw(1.60), -0.285, 0.075, 0.075, 0.40, 8, 'x');       // 巻いた寝具
    box(bT, CC2, 0, tw(1.18), -0.285, 0.24, 0.13, 0.16, { bz: 0.8 });
    box(bT, CAD, -0.17, tw(1.515), -0.30, 0.07, 0.07, 0.07);
    box(bT, CAD, -0.175, tw(1.675), -0.345, 0.028, 0.34, 0.028, { rx: -0.22 }); // アンテナ
    box(bT, CRU, 0.175, tw(1.44), -0.30, 0.09, 0.18, 0.10, { rz: 0.1 });        // 予備の筒

    /* 首と頭 */
    box(bT, CAD, 0, tw(1.565), -0.01, 0.16, 0.11, 0.17);
    box(bT, CA, 0, tw(1.67), 0, 0.27, 0.22, 0.30, { tx: 0.92, tz: 0.94, ct: CA2 });
    box(bT, CA2, 0, tw(1.795), -0.005, 0.25, 0.06, 0.28, { tx: 0.86, tz: 0.88 });
    /* 頂部の稜線：前後に走らせる。真横から見ても真後ろから見ても頭が尖る */
    box(bT, CT2, 0, tw(1.832), -0.01, 0.055, 0.055, 0.30, { tx: 0.35, tz: 0.80, ct: CT });
    /* 後頭部の跳ね上げ（襟足の防護板）。背面の輪郭に段を作る */
    box(bT, CA, 0, tw(1.615), -0.185, 0.29, 0.16, 0.10, { rx: 0.38, tx: 0.85, ct: CA2 });
    /* 面：奥まった暗い帯＋その上に明るいバイザー縁＝向きが一目で分かる */
    box(bT, CAD2, 0, tw(1.655), 0.152, 0.23, 0.10, 0.05);
    box(bT, CT, 0, tw(1.722), 0.158, 0.25, 0.05, 0.06, { ct: CT });
    box(bT, CA, 0.135, tw(1.635), 0.05, 0.045, 0.17, 0.23);
    box(bT, CA, -0.135, tw(1.635), 0.05, 0.045, 0.17, 0.23);
    box(bT, CAD, 0, tw(1.552), 0.085, 0.19, 0.08, 0.17, { tz: 0.9 });
    box(bT, CM, 0, tw(1.545), 0.175, 0.10, 0.07, 0.06);   // 呼吸器の缶

    /* 肩当て：外へ張り出し、下へ向かって広がる。首との間に切れ込みが残る */
    function pauldron(s) {
      box(bT, CA, 0.325 * s, tw(1.445), 0, 0.20, 0.26, 0.36,
        { rz: -0.20 * s, bx: 1.20, tz: 0.86, ct: CA2 });
      box(bT, CAD, 0.360 * s, tw(1.285), 0, 0.17, 0.10, 0.32, { rz: -0.30 * s, bz: 0.9 });
      box(bT, CT2, 0.322 * s, tw(1.575), 0, 0.21, 0.045, 0.34, { rz: -0.19 * s, ct: CT });
    }
    pauldron(1); pauldron(-1);

    /* ---- 腕（ローカル原点＝肩関節。y は下向きに伸びる） ------------------ */
    function arm(B, s) {
      box(B, CAD, 0, -0.11, 0, 0.185, 0.24, 0.21, { bx: 0.92, ct: CAD });
      box(B, CC, -0.012 * s, -0.28, 0.005, 0.155, 0.14, 0.17);
      box(B, CA, -0.030 * s, -0.372, 0.012, 0.175, 0.12, 0.19, { ct: CA2 });
      box(B, CT2, -0.045 * s, -0.425, 0.015, 0.185, 0.04, 0.195);
      box(B, CA, -0.058 * s, -0.485, 0.020, 0.160, 0.15, 0.175, { tx: 1.05 });
      box(B, CAD, -0.085 * s, -0.578, 0.045, 0.130, 0.13, 0.170);
    }
    arm(bAR, 1);
    arm(bAL, -1);
    ...
    /* ---- 脚（ローカル原点＝股関節） -------------------------------------- */
    function leg(B, s) {
      box(B, CAD, 0, -0.045, 0, 0.24, 0.16, 0.28, { ct: CAD });
      box(B, CC, 0.005 * s, -0.225, 0, 0.235, 0.34, 0.26, { bx: 0.88, bz: 0.92 });
      box(B, CA, 0.005 * s, -0.205, 0.135, 0.19, 0.26, 0.06, { ct: CA2 });
      /* 右は雑嚢、左は拳銃嚢。左右で違う出っ張り＝向きが分かる非対称 */
      if (s > 0) box(B, CC2, 0.135, -0.24, 0.005, 0.085, 0.20, 0.17, { rz: -0.06 });
      else box(B, CAD, -0.135, -0.255, 0.02, 0.095, 0.23, 0.13, { rz: 0.06 });
      box(B, CA, 0.005 * s, -0.44, 0.02, 0.21, 0.13, 0.23, { tz: 0.95, ct: CA2 });
      box(B, CT2, 0.005 * s, -0.452, 0.135, 0.13, 0.09, 0.06);
      box(B, CC, 0.005 * s, -0.60, 0, 0.19, 0.28, 0.21);
      box(B, CA, 0.005 * s, -0.60, 0.115, 0.17, 0.30, 0.06, { ct: CA2 });
      box(B, CAD, 0.005 * s, -0.78, 0, 0.175, 0.10, 0.19);
      box(B, CAD, 0.005 * s, -0.865, 0.02, 0.22, 0.15, 0.30, { ct: CAD });
      box(B, CAD, 0.005 * s, -0.895, 0.20, 0.19, 0.09, 0.12, { tz: 0.85 });
      box(B, CAD, 0.005 * s, -0.90, -0.135, 0.18, 0.08, 0.08);
      box(B, CAD2, 0.005 * s, -0.925, 0.03, 0.225, 0.03, 0.325);
    }
    leg(bLR, 1);
    leg(bLL, -1);
    ...
    box(bG, CAD, 0, gy(-0.115), 0.525, 0.070, 0.16, 0.085, { rx: 0.12 });          // 前握把
    ...
    /* =========================================================================
       ...
       ====================================================================== */
    if (!ARML_OWN_MESH) mergeInto(bT, bAL, -0.28, tw(Y_SHLD), 0, -0.20);
    ...
    var armR = new T.Group(); armR.position.set(0.28, Y_SHLD - Y_TORSO, 0); torso.add(armR);
    armR.add(meshOf(bAR));
    /* 構えの初期姿勢。syncRig が毎フレーム上書きするが、
       単体検証の絵が実戦の絵と一致するようゲーム内の ready と同じ値を入れる */
    armR.rotation.set(-1.25, 0, 0.25);

    var armL = new T.Group(); armL.position.set(-0.28, Y_SHLD - Y_TORSO, 0); torso.add(armL);
    if (ARML_OWN_MESH) { armL.add(meshOf(bAL)); armL.rotation.z = -0.20; }

    var legR = new T.Group(); legR.position.set(0.155, Y_HIP, 0); body.add(legR);
    legR.add(meshOf(bLR));
    var legL = new T.Group(); legL.position.set(-0.155, Y_HIP, 0); body.add(legL);
    legL.add(meshOf(bLL));

    /* 銃は Group。スケールは持たせない。+Z が銃口方向 */
    var gun = new T.Group(); gun.position.set(0.26, Y_GUN - Y_TORSO, 0); torso.add(gun);
    gun.add(meshOf(bG));
