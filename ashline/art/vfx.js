...
(function (g) {
  ...
  ASH.vfx = function (T, scene) {
    ...
    function cone(maxAng, bias) {
      var a = maxAng * Math.pow(rnd(), bias);
    ...
    function muzzle(x, y, z, dx, dy, dz) {
      ...
      for (i = 0; i < 2; i++) {
        ...
        E.grow = 0.45;
        E.fade = 0.7;
        if (i === 0) { tint(COL.glow, 1.0); sizeU(0.44, 0.74); E.a0 = 0.95; }
        else { tint(COL.core, 1.0); sizeU(0.22, 0.36); E.a0 = 1.0; }
        emit(ADD);
      }
      /* 発光の芯。ブルームの種。花弁より一段小さく、一段明るく。 */
      reset();
      E.x = x + dx * 0.04; E.y = y + dy * 0.04; E.z = z + dz * 0.04;
      E.tile = TILE_HOT; tint(COL.core, 1.15); sizeU(0.26, 0.40);
      E.ttl = 0.055; E.grow = 0.5; E.a0 = 1.0; E.fade = 0.8;
      emit(ADD);
      /* 外周の柔らかい滲み。これが無いと花弁が紙の切り抜きに見える。 */
      reset();
      E.x = x + dx * 0.06; E.y = y + dy * 0.06; E.z = z + dz * 0.06;
      E.tile = TILE_GLOW; tint(COL.glow, 0.55); sizeU(0.7, 1.05);
      E.ttl = 0.062; E.grow = 0.4; E.a0 = 0.5; E.fade = 1.0;
      emit(ADD);
    ...
    function impactWorld(x, y, z) {
      ...
      E.tile = TILE_HOT; tint(COL.coldSpark, 0.8); sizeU(0.20, 0.42);
      E.ttl = 0.075; E.grow = 0.45; E.a0 = 0.85; E.fade = 1.0;
      emit(ADD);

      /* 2層目：粉塵の噴出。法線まわり ±40° の狭い円錐＝縦に立つプルーム。
         ここでいちばん重要なのは「初期サイズ」。1フレーム目に見えるのは
         ほぼ s0 なので、s0 が小さいと着弾の瞬間が点にしかならない。
         発生位置も法線方向にばらけさせて、最初の1枚から塊にする。 */
      for (i = 0; i < 18; i++) {
        cone(0.72, 0.55);
        o = rr(0.05, 0.46);
        sv = rr(2.4, 6.4);
