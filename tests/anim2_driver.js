...
(async()=>{
  ...
  { const names=['drawBitter','drawPapipoo','drawShark','drawPirate','drawNoroinu','drawCactus','drawWarpdog'];
    for(const n of names){
      ...
      if(src.indexOf('foeStep') >= 0) throw new Error(n+' still uses foeStep (矩形脚の平行移動が残存)');
      if(src.indexOf('foeLegs') < 0) throw new Error(n+' does not call foeLegs (二関節脚に置換されていない)');
    }
  ...
  { let bent=0, samples=0;
    ...
    if(!(bent >= samples*0.3)) throw new Error('knee almost never bends: '+bent+'/'+samples);
  ...
  { const e={x:0,px2:0,dir:1,z:0,state:'walk',gait:0,gaitW:1,anim:0,bob:0,type:'wolf',hp:10,maxHp:10};
    ...
    if(!(g>0.05)) throw new Error('forward motion did not advance the gait');
    if(!(g2 < g - 0.05)) throw new Error('backward motion did not reverse the gait phase (g='+g+' g2='+g2+')');
    // 実装が符号付きであることをソースからも確認
    const src=updateGait.toString();
    if(src.indexOf('mv=_dx*(e.facing||1)') < 0) throw new Error('gait integration is not direction-signed');
    if(src.indexOf('Math.abs(mv)>0.12') < 0 && src.indexOf('Math.abs(mv) > 0.12') < 0) throw new Error('stepping gate still uses unsigned mv');
    console.log('歩行位相の符号 OK (前進 +'+g.toFixed(3)+' → 後退で '+g2.toFixed(3)+' へ巻き戻る)'); }

  // ===== 4) 四つ足の歩容：全脚が同時に垂直になる「テーブル脚」を作らない =====
  { const src=drawBeast.toString();
    if(src.indexOf('PH')<0 || src.indexOf('gaitStep')<0) throw new Error('drawBeast が歩速依存の歩容（PH）／接地サイクルを使っていない');
    ...
    if(calls.length!==4) throw new Error('expected 4 leg calls in drawBeast, got '+calls.length);
    const hx=calls.map(c=>c.hx).sort((a,b)=>a-b);
    if(!(hx[1]<0 && hx[2]>0)) throw new Error('hips are not split front/back: '+hx.join(','));
    ...
    for(const ty of ['boar','hyena']){ const T=ETYPE[ty];
      ...
      if(flat>0) throw new Error(ty+': 全脚が同時に垂直になる位相が '+flat+'/120 存在する（テーブル脚）');
      if(!(worst>6)) throw new Error(ty+': 4本の足先の開きが最小 '+worst.toFixed(1)+'px しかない');
    }
  ...
  { const origLegTo=foeLegTo, origSolve=foeLegSolve;
    ...
    if(measured<11) throw new Error('計測できた敵が '+measured+' 体しかない');
    if(!(bonePulse<0.01)) throw new Error('骨長がフレーム間で '+bonePulse.toFixed(2)+'% 伸縮 ('+boneN+')');
    if(!(worst<0.06)) throw new Error('接地足がワールド座標で '+worst.toFixed(3)+'px/F 滑走 ('+worstN+')');
    if(!(fastest<=7.6)) throw new Error('ケイデンスが速すぎる（ミシン脚）: '+fastest.toFixed(1)+'歩/秒 ('+fastN+')');
    if(!(slowest>=1.2)) throw new Error('ケイデンスが遅すぎる: '+slowest.toFixed(1)+'歩/秒 ('+slowN+')');
  ...
  { let worst=0;
    ...
    if(!(worst<30*4)) throw new Error('遊脚と接地相の水平速度が不連続（しゃくり）: '+worst.toFixed(1));
  ...
  { const e={_lr:17, gait:0, gaitW:1, _stride:16};
    ...
    if(lens.size!==1) throw new Error('骨長がフレームごとに変化している: '+[...lens].join(' , '));
    // フィットが実際に「歩幅の両端に届く長さ」を返すこと
    const h=22-11*0.42, F=legFit({},'x',h,9,9,26);
    if(!(F.R>=Math.hypot(h,13)/0.97-0.01)) throw new Error('歩幅の端に届かない脚長を返した');
  ...
  { // 届く範囲の目標には誤差ゼロで到達する
    ...
    if(!(worst<0.01)) throw new Error('IK の足先誤差 '+worst.toFixed(3)+'px');
    // 接地相は必ず lift=0（地面から浮かない）
    for(let n=0;n<200;n++){ const u=n/200; const f=gaitStep(u,30,8);
      if(f.st && f.y!==0) throw new Error('接地相で足が地面から離れている u='+u.toFixed(3)+' y='+f.y); }
    // 遊脚相はきちんと持ち上がる
    let peak=0; for(let n=0;n<200;n++){ const f=gaitStep(n/200,30,8); if(!f.st) peak=Math.max(peak,-f.y); }
    if(!(peak>6)) throw new Error('遊脚が持ち上がらない peak='+peak);
  ...
  { setupRoster('inu'); startGame(); state='play';
    ...
    if(atPass===null) throw new Error('パッシング（支持脚が真下）の位相が見つからない');
    if(!(atPass>atSplit+1.2)) throw new Error('重心がパッシングで最高になっていない (pass='+atPass.toFixed(2)+' min='+atSplit.toFixed(2)+')');
    // 描画側が同じ値を使っていること
    const ds=drawEnemy.toString();
    if(ds.indexOf('-(e.bobY||0)')<0) throw new Error('drawEnemy が bobY を使っていない（脚のIKと基準面がずれる）');
    ...
    { let worst=9, worstN='', best=0;
      ...
      if(!(worst>3.5)) throw new Error('重心の振れ幅が体高の '+worst.toFixed(1)+'% しかない ('+worstN+')');
      if(!(best-worst<1.5)) throw new Error('重心の振れ幅が個体でばらついている ('+worst.toFixed(1)+'〜'+best.toFixed(1)+'%)');
  ...
  { for(const fn of [drawBeast, foeLegsGait]){ const src=fn.toString();
      if(src.indexOf('reach*0.95')>=0 || src.indexOf('_lim')>=0)
        throw new Error(fn.name+' が脚ごとに歩幅をクランプしている（短い脚だけが滑る）');
      if(src.indexOf('e._stride')<0) throw new Error(fn.name+' が個体共通の歩幅を使っていない'); }
    ...
    for(const q of [[-19,10.5,9,8],[-17,9,8,7],[-21,11,9,8],[-19,9.5,8,7]]){
      ...
      if(!(F.R*0.995>=need)) throw new Error('歩幅の端に届かない: R='+F.R.toFixed(2)+' need='+need.toFixed(2)); }
  ...
  { const targets=['drawBitter','drawBoss','drawGuard0','drawRival','drawGolux','drawDarkKnight','drawAlienBoss'];
    ...
    for(const n of targets){ if(typeof global[n]!=='function' && typeof eval('typeof '+n)==='undefined') continue;
      ...
      if(src.indexOf('foeArm(')<0) throw new Error(n+' has a raw weapon-angle ternary (1フレーム瞬間移動が残存)');
      wired++; }
    if(wired<4) throw new Error('only '+wired+' weapon arms were smoothed');
    // 補間器そのものの性質：目標が飛んでも1フレームでは飛ばない／1フレーム1回だけ進む
    const e={}; rframe=1000;
    if(foeArm(e,0,0.3)!==0) throw new Error('foeArm did not snap on first sight');
    rframe=1001; const a1=foeArm(e,2.0,0.3);
    if(!(a1>0.01 && a1<1.2)) throw new Error('foeArm jumped or froze: '+a1);
    const a1b=foeArm(e,2.0,0.3);
    if(a1b!==a1) throw new Error('foeArm advanced twice in one frame ('+a1+' → '+a1b+')');
    rframe=1010; const a2=foeArm(e,2.0,0.3);      // 長く見えていなければ瞬時に合わせる
    if(a2!==2.0) throw new Error('foeArm did not re-snap after a gap: '+a2);
  ...
  { setupRoster('inu'); startGame(); state='play';
    ...
    if(A.length<40) throw new Error('腕の記録が足りない: '+A.length);
    ...
    if(!(r<=-0.85)) throw new Error('腕が手前脚と対側位相になっていない (相関 '+r.toFixed(3)+'／対側なら負)');
    ...
    if(!((hi-lo)>8)) throw new Error('肩の前後動が '+(hi-lo).toFixed(1)+'px しかない（腕振りが見えない）');
    console.log('腕振りの位相 OK (肩の前後動との相関 '+r.toFixed(3)+' ＝対側／振れ幅 '+(hi-lo).toFixed(1)+'ローカルpx)'); }

  // ===== 5c) 上体の揺れが接地ロックを壊さない =====
  { const src=drawEnemy.toString();
    if(src.indexOf('ctx.rotate(-_f.x*')<0) throw new Error('上体の揺れが入っていない');
    ...
    if(!(worst<0.35)) throw new Error('上体の揺れで足が '+worst.toFixed(2)+'px 動く');
  ...
  { enemies.length=0; encounters.length=0;
    ...
    if(!(moved>10)) throw new Error('walking enemy gait barely advanced ('+moved+'/60)');
