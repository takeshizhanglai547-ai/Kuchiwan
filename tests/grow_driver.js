...
(async()=>{
  ...
  setProgress(0,1);
  if(foeGrowth()!==0) throw new Error('growth should start at 0, got '+foeGrowth());
  if(Math.abs(foeHpGrow()-1)>1e-9 || Math.abs(foeDmgGrow()-1)>1e-9) throw new Error('multipliers should start at 1');
  const g0=foeGrowth();
  setProgress(3,1); const gStage=foeGrowth();
  setProgress(0,7); const gLevel=foeGrowth();
  setProgress(3,7); const gBoth=foeGrowth();
  if(!(gStage>g0)) throw new Error('clearing stages did not raise growth');
  if(!(gLevel>g0)) throw new Error('levelling up did not raise growth');
  if(!(gBoth>gStage && gBoth>gLevel)) throw new Error('growth does not combine both axes');
  console.log('成長度 OK (初期0 / 3面制覇='+gStage.toFixed(1)+' / LV7='+gLevel.toFixed(1)+' / 両方='+gBoth.toFixed(1)+')');
  // 上限で頭打ちになる（無限インフレしない）
  setProgress(99,99);
  if(foeGrowth()>15.0001) throw new Error('growth is not capped: '+foeGrowth());
  if(foeHpGrow()>4 || foeDmgGrow()>2.5) throw new Error('multipliers grew out of hand');
  ...
  if(!(zHi>zLo*2)) throw new Error('zako HP barely grew: '+zLo+' -> '+zHi);
  if(!(bHi>bLo*1.4)) throw new Error('boss HP barely grew: '+bLo+' -> '+bHi);
  if(bHi>bLo*3) throw new Error('boss HP grew too much: '+bLo+' -> '+bHi);
  ...
  if(dLo<=0||dHi<=0) throw new Error('zako attack did not connect');
  if(!(dHi>dLo*1.5)) throw new Error('zako damage barely grew: '+dLo+' -> '+dHi);
  console.log('攻撃力 OK (同じ雑魚の一撃 '+dLo+'→'+dHi+')');

  // ===== 3) 精鋭（階級）が終盤で湧き、称号ぶんだけ強くなる =====
  setProgress(0,1);
  if(foeTier()!==0) throw new Error('elites should not appear at the very start');
  { enemies.length=0; for(let i=0;i<60;i++) spawnEnemy('wolf', p.x+200, LANE);
    if(enemies.some(e=>e.rank>0)) throw new Error('an elite spawned at tier 0'); }
  setProgress(9,16);
  const tier=foeTier();
  if(tier<3) throw new Error('tier too low late in the run: '+tier);
  { enemies.length=0; for(let i=0;i<300;i++) spawnEnemy('wolf', p.x+200, LANE);
    const elites=enemies.filter(e=>e.rank>0);
    if(!elites.length) throw new Error('no elites spawned at tier '+tier);
    if(elites.some(e=>e.rank>tier)) throw new Error('elite rank exceeded the tier');
    const plain=enemies.find(e=>!e.rank);
    if(plain && !elites.some(e=>e.maxHp>plain.maxHp)) throw new Error('elites are not tougher than plain foes');
  ...
  { enemies.length=0; for(let i=0;i<80;i++){ spawnEnemy('garm', p.x+200, LANE); spawnEnemy('bikewan', p.x+200, LANE); spawnEnemy('hbInu', p.x+200, LANE); }
    if(enemies.some(e=>e.rank>0)) throw new Error('bosses/vehicles/hero-bosses must not get a rank'); }
  ...
  { setProgress(9,16); enemies.length=0; spawnEnemy('wolf', p.x+30, LANE);
    ...
    if(!(d3>d0*1.5)) throw new Error('elite damage bonus too small: '+d0+' -> '+d3);
  ...
  { setProgress(9,16); enemies.length=0; spawnEnemy('wolf', p.x+200, LANE);
    ...
    killEnemy(e2); if(!(score>s0 && p.xp>x0)) throw new Error('elites give no extra reward');
  ...
  if(!(think1>think0*1.2)) throw new Error('enemies did not get quicker: '+think0+' -> '+think1);
  if(foeSpdGrow()>1.4) throw new Error('speed growth is too extreme');
  ...
  { setProgress(0,1); lastFoeTier=0; stageBanner=0; stageBannerTxt='';
    checkFoeTierUp();
    if(stageBanner!==0) throw new Error('tier-up notice fired at tier 0');
    setProgress(9,16); checkFoeTierUp();
    if(stageBanner<=0 || !/敵が強化された/.test(stageBannerTxt)) throw new Error('tier-up notice did not fire');
    const txt=stageBannerTxt; stageBanner=0; stageBannerTxt='';
    checkFoeTierUp();
    if(stageBanner!==0) throw new Error('tier-up notice repeated without a tier change');
  ...
  { setProgress(4,10); const g1=foeHpGrow()*diffHpMul();
    ...
    if(!(g3>g1)) throw new Error('lap multiplier no longer stacks with the growth system'); }
