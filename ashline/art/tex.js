...
(function (g) {
  ...
  ASH.tex = function (T) {
    ...
    function buildConcrete(mode) {
      ...
      /* 反復対策のマクロむら。
         注意：ここは一度「低周波で大きく振る」方向に振り切って失敗している。
         タイルに1つしかない大きな特徴は、敷き詰めた瞬間に最も目立つ目印になる。
         正しい方針は逆で、
             ・タイル内に1〜2個しかない大きな特徴を作らない
             ・中〜高周波の情報量を増やして「面」として見せる
             ・低周波は"わずかに"だけ入れて平坦さを避ける
         なので振幅は控えめ（±12%程度）に留める。 */
      var macro = fbm2(S, 3, 4, 4, 5050);
      ...
      if (isH) {
        ...
        fill(o, function (px, py, u, v, out) {
          ...
          /* 濡れ残りの染み：低周波の谷にだけ溜める。
             ここに v の単調関数を掛けるとタイルの上下端で段差になるので使わない
             （継ぎ目テストで検出。面の上下は雨だれと段差の明暗で読ませる）。 */
          lerp3(out, cWet, smoothstep(0.40, 0.14, big[i]) * 0.46, out);
          scl(out, 0.88 + 0.24 * macro[i]);       // マクロむら（弱く。強いと反復の目印になる）
        });
      }
      ...
      for (j = 0; j < 4; j++) {
        var by = j * 128 + rr(-3, 3);
        if (by < 4) by += S;                      // 上端に張り付くと巻き戻し側が欠ける
        x.save();
        if (isH) {
      ...
      for (j = 0; j < 2; j++) {
        ...
        if (isH) {
          ...
          if (j === 0) {                          // 巻き戻し側にも同じものを置く
            x.fillStyle = GY(84); x.fillRect(0, S - 2.5, S, 4);
            x.fillStyle = GY(92); x.fillRect(S - 1.5, 0, 3, S);
          }
        } else {
          ledge(o, py0 - 3, sh(P.ash, 1.55), P.grime, 0.46, 0.55, 4);
          if (j === 0) ledge(o, S - 3, sh(P.ash, 1.55), P.grime, 0.46, 0.55, 4);
          x.fillStyle = CA(P.grime, 0.34); x.fillRect(pxv - 1.5, 0, 3, S);
          x.fillStyle = CA(sh(P.concrete, 1.28), 0.22); x.fillRect(pxv + 1.5, 0, 2, S);
          if (j === 0) {                          // 左端に置いた縦目地の残り半分を右端にも
            x.fillStyle = CA(P.grime, 0.34); x.fillRect(S - 1.5, 0, 3, S);
            x.fillStyle = CA(sh(P.concrete, 1.28), 0.22); x.fillRect(S + 1.5, 0, 2, S);
          }
        }
      }
      ...
      if (!isH) {
        ...
        for (i = 0; i < spalls.length; i++)
          (function (sp) {
            tiled(o, function () {
              crack(o, sp[0], sp[1], rr(0, 6.28), rr(60, 150), 2.2, P.grime, sh(P.concrete, 1.30), 2, 0.22);
            });
          })(spalls[i]);
    ...
    function buildPlaster() {
      ...
      /* 剥離の周波数。3〜4 だとタイルに島が2〜3個しかできず、敷き詰めたとき
         その島の形がそのまま迷彩柄として反復した（実機の中景で発生）。
         周波数を上げて島を小さく多くし、「模様」ではなく「肌」に変える。 */
      var m = warp(S, fbm(S, 8, 5, 909), wx, wy, 40);
      var det = fbm(S, 90, 3, 1201);
      var mott = fbm(S, 10, 4, 331);
      var mid = fbm(S, 24, 4, 332);               // 中周波。遠景で消えないための情報
      var macro = fbm2(S, 3, 4, 4, 5353);
      ...
      function substrate(u, v, i, out) {
        ...
        if (bx < 0.045 || bx > 0.955 || by < 0.10 || by > 0.90) {
          ...
          /* 石・煉瓦と同じく、溝の上下を通しの g で定義して段の境で連続にする。
             上端だけに影を入れると煉瓦の境ごとに段差ができ、それがタイルの
             上下端に露出して継ぎ目になる（継ぎ目テストで検出）。 */
          if (by < 0.10 || by > 0.90) {
            var gj = sat((by > 0.5) ? (by - 0.90) / 0.20 : 0.5 + by / 0.20);
            lerp3(out, cSh, 0.50 * (1 - gj), out);
          }
        }
      }
      ...
      fill(o, function (px, py, u, v, out) {
        ...
        if (mv > T2 + 0.014) {                    // 仕上げ塗り（生きている面）
          /* 迷彩柄対策の本丸。生きた漆喰(L≈146)と露出下地(L≈82)の明度比が
             1.8倍もあると、敷き詰めた瞬間に「明るい島／暗い地」の2値模様として
             読まれる。比を 1.3 倍程度まで詰め、剥離の読み取りは
             ①小口の影 ②欠けた縁の光 ③下地の煉瓦目地 の3つに任せる。
             明度差に頼るのをやめる、というのがここでの判断。 */
          lerp3(cTopD, cTop, 0.22 + 0.98 * mott[i], out);
          scl(out, 0.62 + 0.22 * det[i]);
        } else if (mv > T2 - 0.003) {             // 剥がれ際の割れ肌（明）
          /* ここを太く明るくすると島の全周に白い縁取りが付き、輪郭線に見える。
             帯を細くし、det で途切れさせて「欠けた所だけ光る」ようにする。 */
          lerp3(cTop, cLip, 0.12 + 0.42 * det[i], out);
          ...
          substrate(u, v, i, out);
          scl(out, 1.24);                         // 下地を持ち上げて漆喰との差を詰める
          lerp3(out, cSh, smoothstep(T1 - 0.055, T1 - 0.005, mv) * 0.50 *
            (above > T1 ? 1 : 0.3), out);
        }
        /* 中周波を全面に強めに重ねる。これが無いと遠景で MIP が効いたとき
           「大きな明暗の斑」だけが残り、迷彩柄に見える（実機で発生した）。 */
        scl(out, 0.82 + 0.34 * mid[i]);
        scl(out, 0.90 + 0.20 * macro[i]);
      });
      ...
      for (i = 0; i < 150; i++) {
        ...
        if (mv > T2 - 0.02 && mv < T2 + 0.03)
          (function (a2, b2) {
            tiled(o, function () { chip(o, a2, b2, rr(2, 6), sh(P.plaster, 1.16), P.grime, 0.5); });
          })(px2, py2);
      }
      ...
      for (i = 0; i < 16; i++) {
        ...
        (function (a2, b2) {
          tiled(o, function () {
            crack(o, a2, b2, rr(0, 6.28), rr(40, 160), 1.6, P.grime, sh(P.plaster, 1.32), 2, 0.45);
          });
        })(px2, py2);
      }
      /* --- 雨だれ：起点は面内に散らす。タイルの上端に一斉に並べると、
         下端に対応するものが無く継ぎ目になる（継ぎ目テストで検出）。 --- */
      seed(6161);
      for (i = 0; i < 40; i++) {
        (function (cx, cy, L, w, a, pale) {
          tiled(o, function () {
            drip(o, cx, cy, L, w, pale ? sh(P.plaster, 1.45) : P.grime, a, 0.28, true);
          });
        })(rnd() * S, rnd() * S, rr(50, 300), rr(1.6, 8), rr(0.14, 0.36), rnd() < 0.2);
      }
      for (i = 0; i < 34; i++) {
        px2 = rnd() * S; py2 = rnd() * S;
      ...
      blend(o, greyCanvas(S, ctr(fbm(S, 11, 4, 8181), 1.30, 0.5), 0.70, 1.06), 'multiply', 0.46);
    ...
    function buildStone() {
      ...
      /* 反復対策：低周波は弱く。石ごとの明度差も、敷き詰めると市松模様に
         見えるところまで振ってしまっていたので幅を詰める。 */
      var macro = fbm2(S, 3, 4, 4, 5252);
      ...
      fill(o, function (px, py, u, v, out) {
        ...
        if (bx < jw || bx > 1 - jw || by < jh || by > 1 - jh) {
          ...
          /* 溝の中の上下位置 g（0=溝の天井, 1=溝の底）。
             by>1-jh はこの石の下端＝溝の"上半分"、by<jh は次の石の上端＝溝の"下半分"。
             g を通しで定義することで、石と石の境（＝タイルの上下端でもある）で
             連続になる。以前はここが不連続で継ぎ目として見えていた。 */
          var g = (by > 0.5) ? (by - (1 - jh)) / (2 * jh) : 0.5 + by / (2 * jh);
          g = sat(g);
          lerp3(out, cSh, 0.62 * (1 - g), out);   // 溝の天井＝上の石の小口の裏
          lerp3(out, cAsh, 0.34 * g * g, out);    // 溝の底＝下の石の天端が拾う光
        } else {
          /* 石身：ロットの明度差。0.55〜1.35 まで振ったら中景で市松に見えたので
             0.80〜1.18 に詰めた。近景の情報は目地・欠け・雨だれ側で持たせる。 */
          lerp3(cSD, cSL, sat((det[i] * 0.42 + big[i] * 0.58 - 0.16) / 0.66), out);
          scl(out, 0.80 + 0.38 * hv);
          ...
          /* びしゃん叩き（石工の道具跡）と風化のあばた。
             石の個体差は明度ではなく肌で出す。明度で出すと中景で市松になる。 */
          scl(out, 0.90 + 0.22 * tool[i] * (0.5 + hv));
          scl(out, 0.93 + 0.16 * h2(px >> 1, py >> 1, 9));
        }
        /* 全体の縦方向グラデは弱め。強いと石ごとの差が消えて段々に見える。 */
        scl(out, 0.90 + 0.20 * macro[i]);         // マクロむら（弱く）
      });
    ...
    function buildBrick() {
      ...
      fill(o, function (px, py, u, v, out) {
        ...
        /* 「抜け落ちた煉瓦」は当初 真っ黒な矩形として描いていたが、
           タイルに2個しかない高コントラストの図形なので、敷き詰めると
           水玉模様になって反復が丸見えになった（中景テストで確認）。
           抜けは env.js のジオメトリ側の仕事と割り切り、ここでは
           「深く後退して影に沈んだ煉瓦」までに留める。 */
        var missing = false;
        var sunken = hv2 > 0.93;
        ...
        if (bx < jw || bx > 1 - jw || by < jh || by > 1 - jh) {
          ...
          scl(out, 0.74);
          /* 石と同じく溝の上下を通しで定義する（連続にするため）。 */
          var g = (by > 0.5) ? (by - (1 - jh)) / (2 * jh) : 0.5 + by / (2 * jh);
          g = sat(g);
          lerp3(out, cSh, 0.44 * (1 - g), out);
          lerp3(out, cAsh, 0.20 * g * g, out);
        } else {
          lerp3(cBD, cBL, sat((det[i] * 0.42 + big[i] * 0.58 - 0.18) / 0.62), out);
          scl(out, 0.76 + 0.50 * hv);
          ...
          /* 奥に後退した煉瓦：影に沈むが、穴にはしない */
          if (sunken) { lerp3(out, cSh, 0.42 + 0.22 * big[i], out); }
          if (spall) {
            /* 表面が飛んだ面：焼成前の芯が出て粗い。
        ...
        lerp3(out, cSh, sat(soot[i] * 1.6 - 0.58) * 0.70, out);
        /* マクロむら：目地の格子より低い周波数で、明暗の両方向に振る。
           乗算だけだと平均が落ちて実機で真っ黒になる（一度そうなった）。 */
        scl(out, 0.88 + 0.24 * macro[i]);
      });

      /* 後退した煉瓦の座標を拾い直す（上に煤を出すため） */
      ...
      for (i = 0; i < voids.length; i++) {
        (function (vd) {
          tiled(o, function () {
            ...
            gr.addColorStop(0, CA(P.grime, 0.26));
      ...
      for (i = 0; i < 3; i++)
        (function (a2, b2) {
          tiled(o, function () {
            crack(o, a2, b2, rr(1.2, 1.9), rr(140, 280), 2.8, P.grime, sh(BR, 1.38), 2, 0.28);
          });
        })(rnd() * S, rnd() * S);
    ...
    function buildMetal() {
      ...
      for (i = 0; i < 40; i++) {
        (function (sx, sy) {
          tiled(o, function () {
            var an = rr(-0.30, 0.30), ln = rr(30, 190);
            x.strokeStyle = CA(sh(P.metal, 1.80), rr(0.10, 0.45));
            x.lineWidth = rr(0.7, 2.2); x.lineCap = 'round';
    ...
    function buildRust() {
      ...
      /* 流路の周波数。12 だと縞が太すぎて敷き詰めたとき木の板目に見えた。 */
      var flow = fbm2(S, 22, 4, 5, 3737);
      var flow2 = fbm2(S, 44, 9, 4, 3738);
      ...
      fill(o, function (px, py, u, v, out) {
        ...
        scl(out, 0.64 + 0.72 * sat(flow[i] * 0.7 + flow2[i] * 0.3));
        scl(out, 0.86 + 0.28 * grain[i]);         // 乾き具合のむら（v は使わない）
      });
      ...
      for (i = 0; i < 34; i++) {
        (function (a2, b2, r2) {
          tiled(o, function () {
            chip(o, a2, b2, r2, mix(sh(P.rust, 1.22), P.ash, 0.62), mix(P.rust, P.rebar, 0.75), 0.28);
          });
        })(rnd() * S, rnd() * S, rr(5, 17));
      }
      /* 低周波の濃淡。タイル1枚ぶんの周期でむらを作り、粒の周期を目立たなくする。 */
      blend(o, greyCanvas(S, ctr(fbm2(S, 6, 4, 4, 6969), 1.4, 0.5), 0.70, 1.08), 'multiply', 0.45);
      blend(o, greyCanvas(S, fbm(S, 200, 2, 1414), 0.0, 1.0), 'overlay', 0.10);
      dull(o, P.ash, 0.34);                       // 最後に必ず濁らせる
      return o;
    }
    ...
    function buildGround(mode) {
      ...
      if (isH) {
        ...
        x.save();
        /* 投げ出された灰の環。床は反復回数が最も多いので、丸い明るい環は
           そのまま水玉模様になる。存在は残しつつ濃度を大きく落とす。 */
        var ge = x.createRadialGradient(kx, ky, 40, kx, ky, 110);
        ge.addColorStop(0, CA(sh(P.ash, 1.5), 0.16));
        ...
        gc.addColorStop(0, CA(sh(P.grime, 0.7), 0.42));
        gc.addColorStop(0.45, CA(P.groundDark, 0.30));
      ...
      var ci;
      for (ci = 0; ci < 11; ci++) {
        var ang = ci / 11 * Math.PI * 2 + rr(-0.25, 0.25);
      ...
      if (!isH) {
        ...
        for (i = 0; i < 20; i++) {
          (function (sx, sy) {
            tiled(o, function () {
              var an3 = rr(0, 6.28), ln = rr(40, 170);
              x.strokeStyle = CA(P.groundDark, rr(0.08, 0.24));
              x.lineWidth = rr(4, 14); x.lineCap = 'round';
        ...
        blend(o, greyCanvas(S, ctr(fbm2(S, 4, 5, 4, 2323), 1.2, 0.5), 0.78, 1.04), 'multiply', 0.45);
    ...
    function buildCloth() {
      ...
      var TH = 8;                                  // 糸のピッチ。S(512)を割り切る値でないと織りがタイルしない
      ...
      fill(o, function (px, py, u, v, out) {
        ...
        lerp3(out, cGr, sat(dirt[i] * 1.1 - 0.28) * 0.62, out);
        lerp3(out, cAsh, sat(0.50 - dirt[i]) * 0.24, out);
      });
    ...
    function buildGrime() {
      ...
      fill(o, function (px, py, u, v, out) {
        var i = py * S + px;
        /* 汚れの溜まり。当初はタイルの周縁を暗くしていたが、それは
           「タイルの境界＝面の境界」という誤った前提で、敷き詰めると
           格子が出るうえ継ぎ目にもなった。低周波ノイズの谷に置き換える。 */
        var eg = sat(1.25 - soft[i] * 1.9);
        var m = 1
          - eg * 0.30
          - sat(soft[i] * 1.2 - 0.35) * 0.26
          - sat(fineF[i] - 0.55) * 0.14
          - sat(soft[i] * 1.4 - 0.75) * 0.30;     // 溜まりの濃い所（v は使わない）
