...
```
   ...
   Agent      Agent     Agent   Agent      Agent     Persona
    ...
    prof/relation                    確認ダイアログ  openai / local
             ...
             ┌────────────┼────────────┬────────────┐
             ▼            ▼            ▼            ▼
        Canvas2D    WebGL/WebXR    (Live2D)     (VRM / UE5)
        ← 実装済     ← 実装済        ← 差替口      ← 差替口
```

### §46 の疎結合が実際に効いているか

Avatar は `setSpeaking / setListening / setThinking / lookAt / setBackdrop / canvas()`
しか公開していない。3D/VR レンダラ（`Stage3D`）を足したとき、
**Orchestrator / Memory / Emotion / Voice / Agents は1行も変更していない。**
瞬き・呼吸・視線・ツインテールの物理・リップシンクも 2D 実装のまま動いている。
設計を主張するより、差し替えて壊れなかったことが根拠になる。
...
## STEP 6b — 3D / VR ステージ（§37 Phase 4 の AR/VR を前倒し）

参照した完成イメージ（暖炉のある室内で、対話AIキャラにVRで手を伸ばせる体験）に寄せた、
**Web ネイティブでの実装**。ライブラリを使わず生の WebGL で書いてある。

- **依存ゼロを維持** — Three.js等のCDNを使うと、オフラインとアーティファクトのCSP下で必ず壊れる。
  単一ファイル配布という本リポジトリの制約を守るため、行列演算・シェーダ・XRループを自前で持つ。
- **部屋** — 木の壁・石の背景壁・ラグ・暖炉。テクスチャはすべて Canvas で手続き的に生成（画像ファイル無し）。
- **炎** — 炎の舌を描いたテクスチャ＋2枚の位相違いクアッドで揺らぎ、点光源の強度も同じ位相で明滅させる。
- **キャラクター** — 2D アバターの canvas をそのままテクスチャとして 3D 空間の板に貼る方式。
  低ポリの3Dモデルを新造するより、既に作り込んだ 2D の表情・髪の物理をそのまま活かせる。
- **WebXR** — `immersive-vr` セッション、`local-floor` 基準、両眼レンダリング、コントローラ姿勢の取得。
- **「触れられる」** — コントローラの手が胸の高さに近づくと、視線が手を追い、感情が動き、台詞と表情で反応する。
  ヘッドセットが無い端末でも、**彼女をタップすると同じ反応が起きる**（板を画面に投影して当たり判定を取る実装）。
...
- **VR実機での動作確認**。WebXR のコードは書いたが、この環境にヘッドセットが無いため
  実機での立体視・スケール感・コントローラ追従は**未検証**。ヘッドレスChromiumで検証できたのは
  非VRの3Dビュー（描画・当たり判定・タップ反応）まで
- 3Dキャラクターは板（ビルボード）であり、真の立体ではない。回り込むと厚みが無いことが分かる
- 部屋は1室のみ。移動・ロコモーション無し
