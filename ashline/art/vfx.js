...
(function (g) {
  ...
  ASH.vfx = function (T, scene) {
    ...
    function impactWorld(x, y, z) {
      ...
      /* 外れ弾には「閃光の芯」を置かない。理由は2つあり、どちらも重い。
         (1) 操作：外れ弾の着弾点はレティクルの真下に必ず来る。そこに不透明で
             明るい玉を置くと、撃っている間ずっと狙点が見えない
             ＝狙って撃つゲームが成立しなくなる。
         (2) 読み分け：芯の閃光を「命中側だけが持つもの」にすると、
             白い粉塵（外れ）と暖色の閃光＋飛沫（命中）が、
             色だけでなく「光ったかどうか」でも分かれる。
         外れの契約は「白い粉塵の噴出＋短い火花」であって、閃光ではない。
      ...
      for (i = 0; i < 20; i++) {
        ...
        /* 輪の半径は粒の半径より必ず大きく取る。ただし揃えると
           「白い数珠」に見えるので、半径も粒径もよくばらす。 */
        rs = rr(0.28, 0.60);
        o = rr(0.04, 0.30);             // 面から浮かす量
        sv = rr(2.0, 4.6);              // 法線方向（立ち上がり）
        var so = rr(1.2, 3.2);          // 外向き（広がり）
        reset();
        ...
        /* 粒を大きくして、密度ではなく「裾の重なり」で雲にする。
           TILE_PUFF は中心に濃度が寄っているので、大きくしても
           濃い部分は輪の上に留まり、薄い裾だけが隙間を埋める。
           ＝つながって見えるのに、中心は透けたまま。 */
        sizeU(rr(0.30, 0.52), rr(0.58, 1.00));
        E.ttl = rr(0.45, 0.80); E.grow = 0.42; E.drag = 4.6; E.grav = 0.9;
        /* 不透明度も抑える。粉塵は「厚み」で見せるもので、
           1粒で壁を作るものではない。重なって初めて雲になればよい。 */
        E.a0 = rr(0.32, 0.55); E.fade = 1.6;
        emit(ALP);
      }
      /* 王冠の粒どうしの隙間を埋める薄い粉。
         輪だけだと「白い数珠」に見えて、ひと塊の噴出に見えない。
         内側にも置くが、不透明度を 0.16 まで落としてあるので
         狙点はここを透かして読める（＝密度は稼げるが視界は塞がない）。 */
      for (i = 0; i < 8; i++) {
        rc = rnd() * 6.283;
        tx = uX * Math.cos(rc) + wX * Math.sin(rc);
        ty = uY * Math.cos(rc) + wY * Math.sin(rc);
        ...
        reset();
        E.x = x + tx * rs + nX * o; E.y = y + ty * rs + nY * o; E.z = z + tz * rs + nZ * o;
        E.vx = nX * sv + tx * 1.8; E.vy = nY * sv + ty * 1.8 + 0.6; E.vz = nZ * sv + tz * 1.8;
        E.tile = (i % 2) ? TILE_PUFF2 : TILE_PUFF;
        E.rot = rnd() * 6.283; E.rotv = (rnd() - 0.5) * 2.0;
        tint((i % 2) ? COL.dust : COL.dustCool, 1.0);
        sizeU(rr(0.26, 0.44), rr(0.60, 0.92));
        E.ttl = rr(0.40, 0.68); E.grow = 0.42; E.drag = 4.8; E.grav = 0.9;
        E.a0 = 0.16; E.fade = 1.7;
        emit(ALP);
      }
