...
/* =============================================================================
   PLAYER
   ========================================================================== */
var ST = { FREE: 'FREE', TOCOVER: 'TOCOVER', COVER: 'COVER', ROLL: 'ROLL', SWAP: 'SWAP' };
...
var P = {
  ...
  // anim
  lean: 0, leanV: 0, roll: 0, rollV: 0, stride: 0, crouch: 0
};
...
function coverUpdate(dt, act) {
  ...
  if (act) {
    if (mag > 0.35) startRollOrSwap(lx, ly);
    else leaveCover();
...
function assistScale() {
  ...
  if (P.state === ST.ROLL || P.state === ST.SWAP) return 0;
...
function canFire() {
  ...
  if (P.state === ST.ROLL || P.state === ST.SWAP || P.state === ST.TOCOVER) return false;
  if (P.state === ST.COVER && P.peek < 0.5) return false;   // 隠れたままでは撃てない
...
function shoot() {
  ...
  if (aimTarget) {
    ...
    var dot = clamp(d.x * tx + d.y * ty + d.z * tz, -1, 1);
    var ang = Math.acos(dot);
    var maxA = snapMaxRad();
    if (ang <= maxA && ang > 1e-5) { d.set(tx, ty, tz); snapApplied = ang / DEG; }
  }
  // 拡散
  var sp = currentSpread();
  if (sp > 1e-6) {
    var a = Math.random() * Math.PI * 2, r = Math.sqrt(Math.random()) * sp;
    var up = Math.abs(d.y) < 0.9 ? _tmpV.set(0, 1, 0) : _tmpV.set(1, 0, 0);
    var rt = new T.Vector3().crossVectors(d, up).normalize();
    var uu = new T.Vector3().crossVectors(rt, d).normalize();
    d.addScaledVector(rt, Math.cos(a) * r).addScaledVector(uu, Math.sin(a) * r).normalize();
  }

  // 1) カメラから着弾点を求める
  var far = 90;
  var tW = rayWorld(camera.position.x, camera.position.y, camera.position.z, d.x, d.y, d.z, far);
  var tE = Infinity, hitE = null, head = false;
  for (var i = 0; i < enemies.length; i++) {
    var e = enemies[i]; if (e.dead) continue;
    var r = enemyRay(e, camera.position.x, camera.position.y, camera.position.z, d.x, d.y, d.z);
    if (r.t < tE) { tE = r.t; hitE = e; head = r.head; }
  }
  var camT = Math.min(tW, tE);
  var ix = camera.position.x + d.x * camT, iy = camera.position.y + d.y * camT, iz = camera.position.z + d.z * camT;

  // 2) 銃口から「その方向へ」改めてトレースし直す。
  //    カメラの射線が通っていても、銃が遮蔽の裏にあれば壁に当たる ＝ 遮蔽を撃ち抜かない。
  //    距離を着弾点までに切ってはいけない（敵の表面＝ちょうど上限距離になり、判定が消える）。
  var m = muzzlePos(_tmpV);
  var mx = ix - m.x, my = iy - m.y, mz = iz - m.z;
  var mL = Math.hypot(mx, my, mz) || 1e-5; mx /= mL; my /= mL; mz /= mL;
  var blocked = rayWorld(m.x, m.y, m.z, mx, my, mz, far);
  var mE = Infinity, mHitE = null, mHead = false;
  for (var j = 0; j < enemies.length; j++) {
    var e2 = enemies[j]; if (e2.dead) continue;
    var r2 = enemyRay(e2, m.x, m.y, m.z, mx, my, mz);
  ...
  var ex = m.x + mx * endT, ey = m.y + my * endT, ez = m.z + mz * endT;
  ...
  lastShot = {
    ...
    camBlockedByWorld: tW < tE, muzzleBlocked: blocked < mE,
    end: { x: ex, y: ey, z: ez }, snapApplied: snapApplied
  };

  if (hitEnemy) {
    damageEnemy(hitEnemy, head ? CFG.fire.dmgHead : CFG.fire.dmg, mx, mz, head);
    hitstop = head ? CFG.hitstop.heavy : CFG.hitstop.light;
  }
  // カメラキック
  CAM.kickP += CFG.fire.kickPitch;
  CAM.kickY += (Math.random() * 2 - 1) * CFG.fire.kickYaw;
}

function muzzlePos(out) {
  var f = new T.Vector3(yawDirX(P.yaw), 0, yawDirZ(P.yaw));
  var r = new T.Vector3(-f.z, 0, f.x);
  var y = CFG.player.chest + 0.10 - P.crouch * 0.35 + (P.peekMode === 2 ? P.peek * CFG.cover.peekRise : 0);
...
function updateAnim(dt) {
  ...
  var crouchT = (P.state === ST.COVER) ? (P.face && P.face.low ? 1 - P.peek : 0.15) : (P.sprint ? 0.55 : 0);
...
function updateCamera(dt) {
  ...
  CAM.py = smooth(CAM.py, CFG.player.chest + (P.peekMode === 2 ? P.peek * CFG.cover.peekRise : 0), 0.05, dt);
...
function syncRig() {
  ...
  r.body.position.set(lateral, -P.crouch * 0.32 + rise, 0);
  r.body.rotation.z = -P.lean * 0.25 - P.roll * 0.55 + (P.peekMode === 1 ? P.peekSide * P.peek * 0.24 : 0);
  r.body.rotation.x = P.lean + P.roll * 0.9 + P.crouch * 0.18;
...
var UI = {
  ...
  frame: function () {
    ...
    // アクションボタンの文脈ラベル
    var lab = '遮蔽';
    if (P.state === ST.COVER) lab = IN.stick.mag > 0.35 ? 'ロール' : '離脱';
    else if (P.sprint || (P.state === ST.FREE && IN.stick.mag > 0.25)) lab = 'ダッシュ';
    ...
    var hideRet = P.sprint || P.state === ST.ROLL || P.state === ST.SWAP ||
      (P.state === ST.COVER && P.peek < 0.5) || P.state === ST.TOCOVER;
    e.reticle.classList.toggle('hide', hideRet);
    e.reticle.classList.toggle('lock', !!aimTarget);
    ...
    if (SET.debug) {
      e.dbg.innerHTML =
        ...
        'state ' + P.state + (P.sprint ? '+SPRINT' : '') + ' peek ' + P.peek.toFixed(2) + '<br>' +
        'assist ' + assistScale().toFixed(2) + (aimTarget ? ' [LOCK]' : '') + '<br>' +
...
function boot() {
  ...
  window.__ASHLINE = {
    ...
    ST: ST,
    tick: function (dt, n) { n = n || 1; for (var k = 0; k < n; k++) update(dt); },
    ...
    teleport: function (x, z, yaw) {
      P.x = x; P.z = z; P.vx = 0; P.vz = 0; P.state = ST.FREE; P.face = null;
      P.peek = 0; P.peekMode = 0; P.sprint = false;
      if (yaw !== undefined) { P.yaw = yaw; CAM.yaw = yaw; }
      CAM.px = x; CAM.pz = z;
    },
    state: function () {
      return {
        state: P.state, sprint: P.sprint, x: P.x, z: P.z, yaw: P.yaw,
